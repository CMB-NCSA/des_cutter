...
import sys
import desthumbsNEW
import desthumbsNEW.astrometry as astrometry
import time
import numpy
import copy
...
def write_lightcurve(args):
    ...
    #how should i change to des
    comment = f"# Lightcurve file created by: des-cutter-{desthumbsNEW.__version__} on {date}\n"
...
def write_manifest(args):
    ...
    #change to DES
    comment = f"Manifest file created by: desthumbs-{desthumbsNEW.__version__} on {date}"
    d = args.__dict__
...
def in_uniform_coverage(ra, dec, field):
    """Returns True/False if a (ra,dec) pair is in an des field"""
...
def fitscutter(filename, ra, dec, cutout_names, rejected_names, lightcurve,
               ...
               stage=False, stage_prefix='desthumbs', obsid_names=False):
    ...
    prefix: string
        The prefix to prepend to the output names (i.e.: des)
    outdir: string
    ...
    get_uniform_coverage: bool
        Select only objects in the DES uniform coverage
